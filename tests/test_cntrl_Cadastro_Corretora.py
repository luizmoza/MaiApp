...
class TestCntrlCorretora:
    ...
    def test_request_setup_crudcycle(self,client,test_object):
        generic_object = cl_Corretora(**{})
        guid = app.tk.generate_guid()

        generic_object.CNPJ = guid
        ...
        response1 = client.post('/Cadastro/Corretora/Insert',content_type='multipart/form-data', data=dados)
        assert response1.status_code == 200
        assert app.json.loads(response1.data.decode('utf-8'))['resposta'] =='Corretora Gravada com Sucesso'
        ...
        dados['UsuarioEmail'] = test_object.Email
        dados['UsuarioAuth'] = test_object.ChaveTrocaSenha
        response2 = client.post('/Cadastro/Corretora/Update',content_type='multipart/form-data', data=dados)
        assert response2.status_code == 200
        assert app.json.loads(response2.data.decode('utf-8'))['resposta'] =='Corretora Gravada com Sucesso'
        response3 = client.post('/Cadastro/Corretora/Load',content_type='multipart/form-data', data=dados)
        assert response3.status_code == 200
        assert app.json.loads(response3.data.decode('utf-8'))['resposta'] =='Ok'
        response4 = client.post('/Cadastro/Corretora/Delete',content_type='multipart/form-data', data=dados)
    ...
    def test_request_setup_search(self,test_object,client):
        ...
        dados['nPerPag'] = '10'
        dados['Search'] = ''
        responsesearch = client.post('/Cadastro/Corretora/Search',content_type='multipart/form-data', data=dados)
