...
class TestCntrlEmissor:
    ...
    def test_request_setup_crudcycle(self,client,test_object):
        generic_object = cl_Emissor(**{})
        guid = app.tk.generate_guid()

        generic_object.Nome = guid
        generic_object.cgc = guid
        generic_object.Rating = ''
        generic_object.Agencia = ''
        generic_object.RatingMAI = ''
        generic_object.Categoria = ''
        generic_object.Grupo = ''
        generic_object.AliasBradesco = ''
        generic_object.AliasItau = ''
        generic_object.AliasMellon = ''
        generic_object.TipoCIA = ''
        generic_object.CategoriaListagem = ''
        generic_object.Nacional = '1'
        generic_object.TipoEmissor = ''
        generic_object.RatingSeP = ''
        generic_object.RatingMoodys = ''
        generic_object.RatingFitch = ''
        ...
        response1 = client.post('/Cadastro/Emissor/Insert',content_type='multipart/form-data', data=dados)
        assert response1.status_code == 200
        assert app.json.loads(response1.data.decode('utf-8'))['resposta'] =='Emissor Gravado com Sucesso'
        ...
        dados['UsuarioEmail'] = test_object.Email
        dados['UsuarioAuth'] = test_object.ChaveTrocaSenha
        response2 = client.post('/Cadastro/Emissor/Update',content_type='multipart/form-data', data=dados)
        assert response2.status_code == 200
        assert app.json.loads(response2.data.decode('utf-8'))['resposta'] =='Emissor Gravado com Sucesso'
        response3 = client.post('/Cadastro/Emissor/Load',content_type='multipart/form-data', data=dados)
        assert response3.status_code == 200
        assert app.json.loads(response3.data.decode('utf-8'))['resposta'] =='Ok'
        response4 = client.post('/Cadastro/Emissor/Delete',content_type='multipart/form-data', data=dados)
    ...
    def test_request_setup_search(self,test_object,client):
        ...
        dados['nPerPag'] = '10'
        dados['Search'] = ''
        responsesearch = client.post('/Cadastro/Emissor/Search',content_type='multipart/form-data', data=dados)
