...
class cl_Cotista(app.base):
    IDCotista:Mapped[int] = mapped_column(primary_key=True,unique=True)
    CGC:Mapped[str]
    Nome:Mapped[str]	
    Tipo:Mapped[str]	
    Banco:Mapped[str]	
...
###############################################################################################################################################
    ...
    def get_id_by_Nome(self):
        ...
        try:
            myQuery = """
            select IDCotista from """ + self.__tablename__ + """
            where Nome = '""" + self.Nome + """'
            """
            dfresult = app.pd.read_sql_query(myQuery, self.__db__.engine)
            if len(dfresult)>0:
                self.IDCotista = dfresult['IDCotista'].iloc[0].item() # Item() para tranformar em int nativo python... pandas devolve um numpy
                vv = True
            else:
                self.IDCotista = 0
