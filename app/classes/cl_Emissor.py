...
class cl_Emissor(app.base):
    IDEmissor:Mapped[int] = mapped_column(primary_key=True,unique=True)
    Rating:Mapped[str]	
    Agencia:Mapped[str]	
    RatingMAI:Mapped[str]	
    ...
    TipoCIA:Mapped[str]	
    CategoriaListagem:Mapped[str]	
    Nacional:Mapped[bool]	
    TipoEmissor:Mapped[str]	
    RatingSeP:Mapped[str]	
    RatingMoodys:Mapped[str]	
    RatingFitch:Mapped[str]	
    ...
    def __init__(self,**kwargs):
        ...
        self.RatingMAI= kwargs['RatingMAI'] if 'RatingMAI' in kwargs else None	
        self.Categoria= kwargs['Categoria'] if 'Categoria' in kwargs else None	
        ...
        self.TipoCIA= kwargs['TipoCIA'] if 'TipoCIA' in kwargs else None
        self.CategoriaListagem= kwargs['CategoriaListagem'] if 'CategoriaListagem' in kwargs else None	
        self.Nacional= kwargs['Nacional'] if 'Nacional' in kwargs else None
        self.TipoEmissor= kwargs['TipoEmissor'] if 'TipoEmissor' in kwargs else None	
        self.RatingSeP= kwargs['RatingSeP'] if 'RatingSeP' in kwargs else None	
        self.RatingMoodys= kwargs['RatingMoodys'] if 'RatingMoodys' in kwargs else None
        self.RatingFitch= kwargs['RatingFitch'] if 'RatingFitch' in kwargs else None	
...
###############################################################################################################################################
    ...
    def get_id_by_Nome(self):
        ...
        try:
            myQuery = """
            select IDEmissor from """ + self.__tablename__ + """
            where Nome = '""" + self.Nome + """'
            """
            dfresult = app.pd.read_sql_query(myQuery, self.__db__.engine)
            if len(dfresult)>0:
                self.IDEmissor = dfresult['IDEmissor'].iloc[0].item() # Item() para tranformar em int nativo python... pandas devolve um numpy
                vv = True
            else:
                self.IDEmissor = 0
