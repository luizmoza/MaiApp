...
class cl_FechamentoIndice(app.base):
    IDFechamentoIndice:Mapped[int] = mapped_column(primary_key=True,unique=True)
    idIndice:Mapped[int]
    Taxa:Mapped[float]	
    NumeroIndice:Mapped[float]	
    Data:Mapped[datetime.datetime]	
    Projecao:Mapped[float]	
    Descricao:Mapped[str]	
    __tablename__ = "FechamentoIndice"
    __table_args__ = {'extend_existing': True}
    __db__ = app.db
    __pk__ = ['IDFechamentoIndice']
    __nn__ = []
    __nnbl__ = []
    __nnint__ = []
    __nndbl__ = []
    ...
    def  __init__(self,**kwargs): 
        self.IDFechamentoIndice = kwargs['IDFechamentoIndice'] if 'IDFechamentoIndice' in kwargs else None
        self.idIndice = kwargs['idIndice'] if 'idIndice' in kwargs else None
        self.Taxa = kwargs['Taxa'] if 'Taxa' in kwargs else None
        self.NumeroIndice = kwargs['NumeroIndice'] if 'NumeroIndice' in kwargs else None
        self.Data = kwargs['Data'] if 'Data' in kwargs else None
        self.Projecao = kwargs['Projecao'] if 'Projecao' in kwargs else None
...
###############################################################################################################################################
    ...
    def get_id_data_idasset(self,db):
        ...
        try:
            myQuery = """
            select IDFechamentoIndice from """ + self.__tablename__ + """
            where data = '""" + self.data.strftime('%Y-%m-%d') + """'
            and idIndice = """ + str(self.idIndice) + """
            """
            dfresult = app.pd.read_sql_query(myQuery, db.engine)
            if len(dfresult)>0:
                self.IDFechamentoIndice = dfresult['IDFechamentoIndice'].iloc[0].item() # Item() para tranformar em int nativo python... pandas devolve um numpy
                vv = True
            else:
                self.IDFechamentoIndice = 0
