...
class cl_Swap(app.base):
    IDSwap:Mapped[int] = mapped_column(primary_key=True,unique=True)	
    idBolsa:Mapped[int]
    idMoeda:Mapped[int]
    Ticker:Mapped[str]
    ...
    def __init__(self,**kwargs): 
        self.IDSwap = kwargs['IDSwap'] if 'IDSwap' in kwargs else None	
...
###############################################################################################################################################
    ...
    def get_id_alias(self,db):
        vv = False
        try:
            myQuery = """
            select IDSwap from """ + self.__tablename__ + """
            where alias = '""" + self.alias + """'
            """
            dfresult = app.pd.read_sql_query(myQuery, db.engine)
            if len(dfresult)>0:
                self.IDSwap = dfresult['IDSwap'].iloc[0].item() # Item() para tranformar em int nativo python... pandas devolve um numpy
                vv = True
            else:
                self.IDSwap = 0
