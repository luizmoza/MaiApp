...
class cl_FechamentoRV(app.base):
    IDFechamentoRV:Mapped[int] = mapped_column(primary_key=True,unique=True)
    idRV:Mapped[int]
    Data:Mapped[datetime.datetime]
    Codigo:Mapped[str]	
    VolumeNegociado:Mapped[float]	
    Pu:Mapped[float]	
    Bonificacao:Mapped[float]	
    Bonus:Mapped[float]	
    Cisao:Mapped[float]	
    Desdobramento:Mapped[float]	
    Dividendo:Mapped[float]	
    Grupamento:Mapped[float]	
    JSCP:Mapped[float]	
    Rendimento:Mapped[float]	
    Restituicao:Mapped[float]	
    Subscricao:Mapped[float]	
    BidAskSpread:Mapped[float]	
    __tablename__ = "FechamentoRV"
    __table_args__ = {'extend_existing': True}
    __db__ = app.db
    __pk__ = ['IDFechamentoRV']
    __nn__ = ['Codigo']
    __nnbl__ = []
    __nnint__ = []
    __nndbl__ = []
...
###############################################################################################################################################
    ...
    def get_id_data_idasset(self,db):
        ...
        try:
            myQuery = """
            select IDFechamentoRV from """ + self.__tablename__ + """
            where data = '""" + self.data.strftime('%Y-%m-%d') + """'
            and IDRV = """ + str(self.IDRV) + """
            """
            dfresult = app.pd.read_sql_query(myQuery, db.engine)
            if len(dfresult)>0:
                self.IDFechamentoRV = dfresult['IDFechamentoRV'].iloc[0].item() # Item() para tranformar em int nativo python... pandas devolve um numpy
                vv = True
            else:
                self.IDFechamentoRV = 0
