...
dictNotMTM = app.pd.read_excel(r"../app/Novo_Gerencial.xlsm", 'Aux',skiprows=[0])
dfData = app.pd.read_excel(r"../app/Novo_Gerencial.xlsm", 'Dt_Param',skiprows=[0])
dfRendimentos = app.pd.read_excel(r"../app/Novo_Gerencial.xlsm", 'QTLink')
...
del dfRendimentos['Data']
############################################### Caixa Rendimentos Quantun #####################################################
###############################################################################################################################
################################################################################################################################
############################################### Lista de fundos e datas para processar #########################################


#data = app.dt.strptime('2024-09-02','%Y-%m-%d')
#listacgcs = ['19718324000198','20833940000179','19366021000153','37467427000104','41544993000140']
#strlistacgcs = f'{*listacgcs,}'

# Pega carteiras batidas liberados nos ultimos 30 dias < D-1
# Tira as carteiras que já foram processadas no gerencial (Guardar no log Robo)
# o que sobrar roda! ... esquece input!

#No primeiro momento eu faço só pra uma lista chumbada aqui
# de datas e de fundos    
...
app.db.connect()
# Descomentar quando terminar de Codar
# app.db.execRawQuery("Delete from Mov where datamov ='" + data.strftime('%Y-%m-%d') + "' and aliasativo like 'Cota#%'")
# app.db.execRawQuery("Delete from Mov where datamov ='" + data.strftime('%Y-%m-%d') + "' and aliasativo like 'SPOT#BRLOTC@REAL%'")
# app.db.execRawQuery("Delete from Mov where datamov ='" + data.strftime('%Y-%m-%d') + "' and aliasativo like '%RF#BRLB3@Compromissada%'")
# app.db.execRawQuery("Delete from Mov where datamov ='" + data.strftime('%Y-%m-%d') + "' and aliasativo like 'RF#BRLB3@TERM%'")
# Descomentar quando terminar de Codar
app.db.drop()
...
#dfCustodiaAutomatica.loc[app.tkstr.left(dfCustodiaAutomatica['aliasativo'].str,5)=="Cota#",'Qtd_Bol'] = dfCustodiaAutomatica['delta_fin']
#dfCustodiaAutomatica.loc[app.tkstr.left(dfCustodiaAutomatica['aliasativo'].str,5)!="Cota#",'Qtd_Bol'] = dfCustodiaAutomatica['delta_qtd']
...
dfCustodiaAutomatica = dfCustodiaAutomatica[dfCustodiaAutomatica['Qtd']!=0]


#### Quando terminar de codar, descomentar!!!! #####
#Mov = cl_Mov(**{})
...
#a = dfCustodiaD_1[dfCustodiaD_1['CGC'] =='11435287000107']
#a = a[a['AliasAtivo'] =='RF#BRLB3@LFT 01/03/2029']
# b = dfPosicao_Cota[dfPosicao_Cota['cgc'] =='04495282000104']
# b = b[b['aliasativo'] =='Cota#BRLOTC@08915927000163']
#sys.exit('PAra!')
...
dfFinal.to_excel(r"../app/Current_PreGerencial.xlsx", index=False)
#dfFinal.to_csv(r"../app/Current_PreGerencial.csv")
